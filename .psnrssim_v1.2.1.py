...
for filename in opt.filenames:
    ...
    if compareimg.size[0] != img.size[0] or compareimg.size[1] != img.size[1]:
        ...
        char_size = (char_size[0]*img.size[0]//lpt_std_size[0],char_size[1]*img.size[1]//lpt_std_size[1])
        Cx = [c*img.size[0]//lpt_std_size[0] for c in Cx]
        Cy = [c*img.size[1]//lpt_std_size[1] for c in Cy]
        Dx = [c*img.size[0]//lpt_std_size[0] for c in Dx]
    ...
    for i,im in enumerate(img_chars):        
        Image.fromarray(im*255).convert("L").save("{0}_{1}.png".format(lpt_num_seq,i)) # debug
    ...
    avgpsnr += psnr
    ssim = compare_ssim(compareimg,img,dynamic_range=max(compareimg.max(),img.max())-min(compareimg.min(),img.min()))#data_range=img.max() - img.min())
    ssim = round(ssim,2)
    avgssim += ssim
...
figwidth = 20
fig = plt.figure(figsize=(figwidth,figwidth*9/16))
plt.title("Chỉ số SSIM trung bình theo mỗi ký tự", fontsize=24)
plt.xlabel("Ký tự", fontsize=20)
plt.ylabel("SSIM trung bình", fontsize=20)
...
figwidth = 20
fig = plt.figure(figsize=(figwidth,figwidth*9/16))
plt.title("Tỉ lệ ảnh có SSIM lớn hơn {} theo mỗi ký tự".format(opt.ssimT), fontsize=24)
plt.xlabel("Ký tự", fontsize=20)
...
figwidth = 20
fig = plt.figure(figsize=(figwidth,figwidth*9/16))
plt.title("Tỉ lệ nhận dạng đúng theo mỗi ký tự", fontsize=24)
plt.xlabel("Ký tự", fontsize=20)
plt.ylabel("%", fontsize=20)
plt.grid(True,axis='y')
ax0 = fig.axes[0]
...
for i,pos in enumerate(corr_recog_char_pos_count.keys()):
    data = np.array(list(corr_recog_char_pos_count[pos].values()))
    axis = np.array(list(corr_recog_char_pos_count[pos].keys()))
