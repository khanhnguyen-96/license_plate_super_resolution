...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="realreal, realsimu, simureal, simusimu",
)
parser.add_argument(
    "--digit-dir", type=str, default="digits", help="Directory contains digit images"
)
parser.add_argument(
    "--testdir",
    type=str,
    required=True,
    help="Test directory contains folders HRS and LR",
)
parser.add_argument(
    "--save-dir",
    type=str,
    required=True,
    help="Directory to save result. If file exists, it will be overwritten.",
)
parser.add_argument(
    "--sep",
    type=str,
    default="_",
    help='"_" or "-", char concat lpt_num and'
    + "a number that distinguish filenames have the same lpt_num.",
)
parser.add_argument(
    "--ssimT", type=float, default=0.82, help="Count SSIM > ssimT (Default is 0.82)"
)
parser.add_argument(
    "filenames",
    metavar="filename",
    type=str,
    nargs="+",
    help="input images to be compute PSNR and SSIM.",
)
...
digits = [
    [
        f.split(".")[:-1][0],
        img_as_float(Image.open(path.join(opt.digit_dir, f)).convert("L")),
    ]
    for f in listdir(opt.digit_dir)
    if is_image_file(path.join(opt.digit_dir, f))
]
digits = sorted(digits)

# Standard parameters of license plate in pixel
lpt_std_size = lpt_utils.lpt_std_size
...
for filename in opt.filenames:
    ...
    if len(lptnum[0]) == 5:  # e.g. 59MĐ1_123.45
        ...
        img_chars = [
            img[L1[i][1] : char_size[1] + L1[i][1], L1[i][0] : char_size[0] + L1[i][0]]
            for i in range(len(lptnum[0]))
        ]
        compareimg_chars = [
            compareimg[
                L1[i][1] : char_size[1] + L1[i][1], L1[i][0] : char_size[0] + L1[i][0]
            ]
            for i in range(len(lptnum[0]))
        ]
    else:
    ...
    if len(lptnum[1]) == 4:  # e.g. 59Z1_1234
        img_chars.extend(
            [
                img[
                    L1[i + len(lptnum[0])][1] : L1[i + len(lptnum[0])][1]
                    + char_size[1],
                    L1[i + len(lptnum[0])][0] : L1[i + len(lptnum[0])][0]
                    + char_size[0],
                ]
                for i in range(len(lptnum[1]))
            ]
        )
        compareimg_chars.extend(
            [
                compareimg[
                    L1[i + len(lptnum[0])][1] : L1[i + len(lptnum[0])][1]
                    + char_size[1],
        ...
        img_chars.extend(
            [
                img[
                    L2[i][1] : L2[i][1] + char_size[1],
                    L2[i][0] : L2[i][0] + char_size[0],
                ]
                for i in range(len(lptnum[1]))
            ]
        )
    ...
    for i, im in enumerate(img_chars):
        ...
        # Image.fromarray(compareimg_chars[i] * 255).convert("L").save(
        #     "{0}_{1}_com.png".format(lpt_num_seq, i)
        # )  # debug
        # continue  # debug
        ...
        ssim = compare_ssim(
            compareimg_chars[i],
            im,
            dynamic_range=max(compareimg_chars[i].max(), im.max())
            - min(compareimg_chars[i].min(), im.min()),
        )
        ...
        # Count ssim > ssimT
        if ssim > opt.ssimT:
            ssimT_char_count[lpt_num_seq[i]] = (
                ssimT_char_count.get(lpt_num_seq[i], 0) + 1
            )
        else:
        ...
        for d in std_chars:
            s = compare_ssim(
                d[1], im
            )  # ,dynamic_range=max(d[1].max(),im.max())-min(d[1].min(),im.min()))
            if max_s < s:
                max_s = s
                max_c = d[0]
            # debug
        ...
        if lpt_num_seq[i] == max_c:
            corr_recog_char_count[lpt_num_seq[i]] = (
                corr_recog_char_count.get(lpt_num_seq[i], 0) + 1
            )
        else:
            corr_recog_char_count[lpt_num_seq[i]] = corr_recog_char_count.get(
                lpt_num_seq[i], 0
            )
    # continue # debug
    ...
    lpt_recog = (
        lpt_recog_seq[: len(lptnum[0])]
        + "_"
        + lpt_recog_seq[len(lptnum[0]) : -2]
        + dot
        + lpt_recog_seq[-2:]
    )
    ...
    lr = Image.open(
        path.join(
            opt.testdir,
            "LR",
            path.basename(filename.replace("_SR" + opt.srext, opt.lrext)),
        )
    )
    ...
    lpt_unique[lpt_num] = lpt_unique.get(lpt_num, 0)
    if lpt_recog == lpt_num and lpt_unique[lpt_num] == 0:
        total_corr_recog_lpt += 1
        lpt_unique[lpt_num] = 1
    ...
    if len(lpt_num) == 8:  # 50Y_1234
        corr_recog_char_pos_count[0][lpt_num[0]] += (
            1 if lpt_recog[0] == lpt_num[0] else 0
        )
        corr_recog_char_pos_count[1][lpt_num[1]] += (
            1 if lpt_recog[1] == lpt_num[1] else 0
        )
        corr_recog_char_pos_count[2][lpt_num[2]] += (
            1 if lpt_recog[2] == lpt_num[2] else 0
        )
        # 3
        # 4
        corr_recog_char_pos_count[5][lpt_num[4]] += (
            1 if lpt_recog[4] == lpt_num[4] else 0
        )
        corr_recog_char_pos_count[6][lpt_num[5]] += (
            1 if lpt_recog[5] == lpt_num[5] else 0
        )
        corr_recog_char_pos_count[7][lpt_num[6]] += (
            1 if lpt_recog[6] == lpt_num[6] else 0
        )
        corr_recog_char_pos_count[8][lpt_num[7]] += (
            1 if lpt_recog[7] == lpt_num[7] else 0
        )
        ...
        corr_recog_char_pos_count[8][lpt_num[8]] += (
            1 if lpt_recog[8] == lpt_num[8] else 0
        )
        corr_recog_char_pos_count[9][lpt_num[9]] += (
            1 if lpt_recog[9] == lpt_num[9] else 0
        )
        ...
        corr_recog_char_pos_count[3][lpt_num[3]] += (
            1 if lpt_recog[3] == lpt_num[3] else 0
        )
        # 4
        corr_recog_char_pos_count[5][lpt_num[5]] += (
            1 if lpt_recog[5] == lpt_num[5] else 0
        )
        corr_recog_char_pos_count[6][lpt_num[6]] += (
            1 if lpt_recog[6] == lpt_num[6] else 0
        )
        corr_recog_char_pos_count[7][lpt_num[7]] += (
            1 if lpt_recog[7] == lpt_num[7] else 0
        )
        corr_recog_char_pos_count[8][lpt_num[8]] += (
            1 if lpt_recog[8] == lpt_num[8] else 0
        ...
        corr_recog_char_pos_count[8][lpt_num[9]] += (
            1 if lpt_recog[9] == lpt_num[9] else 0
        )
        corr_recog_char_pos_count[9][lpt_num[10]] += (
            1 if lpt_recog[10] == lpt_num[10] else 0
        )
        ...
        corr_recog_char_pos_count[4][lpt_num[4]] += (
            1 if lpt_recog[4] == lpt_num[4] else 0
        )
        corr_recog_char_pos_count[5][lpt_num[6]] += (
            1 if lpt_recog[6] == lpt_num[6] else 0
        )
        corr_recog_char_pos_count[6][lpt_num[7]] += (
            1 if lpt_recog[7] == lpt_num[7] else 0
        )
        corr_recog_char_pos_count[7][lpt_num[8]] += (
            1 if lpt_recog[8] == lpt_num[8] else 0
        )
        corr_recog_char_pos_count[8][lpt_num[10]] += (
            1 if lpt_recog[10] == lpt_num[10] else 0
        )
        corr_recog_char_pos_count[9][lpt_num[11]] += (
            1 if lpt_recog[11] == lpt_num[11] else 0
        )
    ...
    ssim = compare_ssim(
        compareimg,
        img,
        dynamic_range=max(compareimg.max(), img.max())
        - min(compareimg.min(), img.min()),
    )
    ssim = round(ssim, 2)
    avgssim += ssim
    testresult.append(
        "{0}: {1}: {2:.2f} / {3:.2f} : {4}\r\n".format(
            path.basename(comparename), path.basename(filename), psnr, ssim, lpt_recog
        )
    )
...
testresult.append(
    "avgpsnr / avgssim: {0:.2f} / {1:.2f}\r\n".format(
        avgpsnr / len(opt.filenames), avgssim / len(opt.filenames)
    )
)
testresult.append(
    "Correct recognized images: {0}/{1} ({2:.2f}%)\r\n".format(
        total_corr_recog_img,
        len(opt.filenames),
        total_corr_recog_img * 100 / len(opt.filenames),
    )
)
testresult.append(
    "Correct recognized lpt: {0}/{1} ({2:.2f}%)\n".format(
        total_corr_recog_lpt,
        len(lpt_unique.keys()),
        total_corr_recog_lpt * 100 / len(lpt_unique.keys()),
    )
)
...
for k in corr_recog_size_count.keys():
    corr_recog_size_count[k] = round(
        corr_recog_size_count[k] * 100 / ssim_size_count[k], 1
    )
...
for pos in corr_recog_char_pos_count.keys():
    for c in corr_recog_char_pos_count[pos]:
        if char_pos_count[pos][c] == 0:
            ...
            corr_recog_char_pos_count[pos][c] = (
                corr_recog_char_pos_count[pos][c] * 100 // char_pos_count[pos][c]
            )
...
#
_, axis, data = sort3list(
    [w * h for w, h in ssim_size_count.keys()],
    ["{0} x {1}".format(w, h) for w, h in ssim_size_count.keys()],
    list(ssim_size_count.values()),
)
figwidth = 22
...
fig = plt.figure(figsize=(figwidth, figwidth * 9 / 16))
plt.title(
    "Tỉ lệ ảnh có SSIM lớn hơn {} theo mỗi ký tự".format(opt.ssimT), y=1.01, fontsize=24
)
plt.xlabel("Ký tự", fontsize=20)
...
#
axis, data = sort2list(
    list(corr_recog_char_count.keys()), list(corr_recog_char_count.values())
)
figwidth = 20
fig = plt.figure(figsize=(figwidth, figwidth * 9 / 16))
...
f.suptitle(
    "Tỉ lệ nhận dạng đúng của mỗi ký tự theo vị trí của ký tự", y=0.94, fontsize=24
)
for i, pos in enumerate(corr_recog_char_pos_count.keys()):
    axis, data = sort2list(
        list(corr_recog_char_pos_count[pos].keys()),
        list(corr_recog_char_pos_count[pos].values()),
    )
    ...
    save_stc(
        path.join(opt.save_dir, "Corr_Position {}.txt".format(pos)),
        axis.tolist(),
        data.tolist(),
    )
...
#
_, axis, data = sort3list(
    [c * t for c, t in corr_num_char_count.keys()],
    ["{0}/{1}".format(c, t) for c, t in corr_num_char_count.keys()],
    list(corr_num_char_count.values()),
)
figwidth = 20
fig = plt.figure(figsize=(figwidth, figwidth * 9 / 16))
...
#
_, axis, data = sort3list(
    [w * h for w, h in corr_recog_size_count.keys()],
    ["{0} x {1}".format(w, h) for w, h in corr_recog_size_count.keys()],
    list(corr_recog_size_count.values()),
)
figwidth = 22
fig = plt.figure(figsize=(figwidth, figwidth * 9 / 16))
plt.title(
    "Tỉ lệ số ảnh nhận dạng đúng trên số ảnh theo kích thước", y=1.01, fontsize=24
)
plt.xlabel("Kích thước của ảnh (pixel)", fontsize=20)
plt.ylabel("%", fontsize=20)
plt.scatter(axis, data)
