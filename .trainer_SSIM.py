...
class Trainer():
            ...
            for idx_img, (lr, hr, _) in enumerate(self.loader_test):
                ...
                if no_eval:
                    ...
                    if self.args.loss == '1*SSIM':
                        eval_acc += pytorch_ssim.ssim(sr,hr.div(rgb_range)).item()
                    else:
    ...
    def _calc_loss(self, sr, hr):
        ...
        for i, l in enumerate(self.loss):
            ...
            loss = -loss if l['type'] == 'SSIM' else loss

            loss_list.append(l['weight'] * loss)
            self.ckp.log_training[-1, i] += -loss.item() if l['type'] == 'SSIM' else loss.item()
