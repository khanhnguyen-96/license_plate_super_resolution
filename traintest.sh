...
then
    W=64
    H=64
    python main.py --model RDN --scale 4 --RDNconfig B --batch_size 16 --model_best_loss 2 --save RDN_D16C8G64_"$W"x"$H"_all_rot_noblur_run2 --epochs 300 --dir_dataLR /content/dataset/SRtrainset_2_x4.lmdb --dir_dataHR /content/dataset/SRtrainset_2_HR.lmdb --datatype simureal --data_test LMDB --data_train LMDB --std-lr-width $W --std-lr-height $H --gauss-std -1 --interp nearest --reset --load RDN_D16C8G64_64x64_all_rot_noblur_run2
elif [ "$1" = "trainerdn" ]
then
    W=69
    H=48
