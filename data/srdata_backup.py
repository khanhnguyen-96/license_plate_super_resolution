...
class SRData(data.Dataset):
    ...
    def target_transform(self, hr, scale, std_lr_width, std_lr_height):
        ...
        return misc.imresize(hr,(std_lr_height*scale, std_lr_width*scale),'bicubic');
    ...
    def input_transform(self, lr, std_lr_width, std_lr_height, gauss_std):
        ...
        if (area > std_area):
            ...
            lr = np.array(lr)
        return misc.imresize(lr,(std_lr_height, std_lr_width),'bicubic')
    ...
    def _get_patch(self, lr, hr):
        ...
        if self.train:
            ...
            lr,_ = common.augment([lr,lr], hflip=False, vflip=False, rot90=False, rot=True, shift=False)
            #lr = common.add_noise(lr, self.args.noise)
        else:
            # ih, iw = lr.shape[0:2]
            # hr = hr[0:ih * scale, 0:iw * scale]
            lr = self.input_transform(lr,self.args.std_lr_width, self.args.std_lr_height, self.args.gauss_std)
            hr = self.target_transform(hr,scale,self.args.std_lr_width, self.args.std_lr_height)
