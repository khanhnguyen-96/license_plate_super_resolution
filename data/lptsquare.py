...
class LPTSQUARE(srdata.SRData):
    def __init__(self, args, train=True):
        self.repeat = args.test_every // (args.n_train // args.batch_size)
        self.datatype = args.datatype
        super(LPTSQUARE, self).__init__(args, train)
    ...
    # LPTSQUARE use datatype = 'simureal' only
    def get_list_hr_lr(self, datatype):
        ...
        if datatype == "realreal" or datatype == "simureal":
            ...
            raise ValueError(
                'datatype is not in "realreal" or "simureal" or "realsimu" or "simusimu"'
            )
        ...
        LRfilenames = [
            ...
            if f[0:2] == "LR" and is_image_file(os.path.join(self.apath, f))
        ]

        # Add all LR images of that license plate number
        for x in lcplt_num:
            x = x.strip("\n")
